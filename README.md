...
The playable package is in `UIFiles/TheBestRanger`. It uses a complete Project Quarm UI file set, with the missing default XML files added so the package is self-contained. The Ranger theme uses dark green tree bark across the UI chrome textures, with custom Ranger class art textures.
...
- `tools/apply-bark-theme.ps1` - reapplies the dark green bark treatment to UI chrome textures.
- `docs/project-quarm-ui-notes.md` - notes on how the XML and image files work together.

## Rebuild Art And Theme

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\generate-ranger-assets.ps1
powershell -ExecutionPolicy Bypass -File .\tools\apply-bark-theme.ps1
```

The generated and themed images intentionally keep filenames already referenced by the XML, such as `wnd_bg_dark_rock.tga`, `wnd_bg_light_rock.tga`, `window_pieces01.tga`, `ranger01.tga`, and `ranger02.tga`, so the skin can override art without rewriting every window definition.
